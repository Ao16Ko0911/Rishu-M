...
function toggleMenu() {
  ...
  function updateCourseList() {
    ...
    // 現在の選択肢をクリア
    courseDropdown.innerHTML = '<option>--未選択--</option>';
  
    // 選択された学年と講義期間に応じた講義を追加
    if (grade in courseData && term in courseData[grade]) {
      courseData[grade][term].forEach(course => {
  ...
  講義: ${course}
  受講年度: ${year}
  総合評価: ${rating === "--未選択--" ? "未選択" : rating}
  楽単レベル: ${ease === "--未選択--" ? "未選択" : ease}
  感想: ${feedback || "なし"}
  `;
